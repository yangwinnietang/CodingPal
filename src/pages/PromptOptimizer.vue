    ...
    <div class="flex h-full">
        ...
        <div class="flex-1 p-6 flex flex-col bg-gray-50">
          ...
          <div class="space-y-4">
            <button 
              ...
              class="w-full btn-primary py-4 text-base font-semibold shadow-lg hover:shadow-xl"
            >
              <i v-if="pageState.isOptimizing" class="i-lucide-loader-2 w-5 h-5 mr-3 animate-spin"></i>
              <i v-else class="i-lucide-sparkles w-5 h-5 mr-3"></i>
      ...
      <div class="w-1/2 flex flex-col">
        ...
        <div class="flex-1 p-6 overflow-y-auto bg-gray-50">
          <!-- 加载状态 -->
          <div v-if="pageState.isOptimizing" class="space-y-6">
            <div v-for="model in pageState.selectedModels" :key="model" class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-3">
                  <div class="w-8 h-8 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-center">
                    <i class="i-lucide-brain w-4 h-4 text-blue-600"></i>
                  </div>
                  <span class="font-semibold text-gray-800">{{ MODEL_CONFIGS[model].displayName }}</span>
                </div>
                <div class="flex items-center space-x-3 text-blue-600">
                  <i class="i-lucide-loader-2 w-5 h-5 animate-spin"></i>
                  <span class="text-sm font-medium">优化中...</span>
                </div>
              </div>
              <div class="bg-gray-50 rounded-xl p-4 animate-pulse border border-gray-200">
                <div class="h-4 bg-gray-200 rounded-lg mb-3"></div>
          ...
          <!-- 优化结果 -->
          <div v-else-if="pageState.results.length > 0" class="space-y-6">
            <div v-for="result in pageState.results" :key="result.id" class="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-200">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-3">
                  <div class="w-8 h-8 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-center">
                    <i class="i-lucide-brain w-4 h-4 text-blue-600"></i>
                  </div>
                  <span class="font-semibold text-gray-800">{{ MODEL_CONFIGS[result.modelName].displayName }}</span>
                </div>
                <div class="flex items-center space-x-3">
                  <span v-if="!result.error" class="text-xs text-green-700 bg-green-50 px-3 py-1 rounded-full border border-green-200">
                    {{ result.responseTime }}ms
                  </span>
                  <span v-if="result.tokensUsed > 0" class="text-xs text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-200">
                    {{ result.tokensUsed }} tokens
                  </span>
                  <span v-if="result.error" class="text-xs text-red-700 bg-red-50 px-3 py-1 rounded-full border border-red-200">
                    失败
                  </span>
                  <button 
                    v-if="!result.error && result.optimizedPrompt"
                    @click="copyResult(result.optimizedPrompt)"
                    class="p-2 hover:bg-gray-100 rounded-xl transition-colors border border-transparent hover:border-gray-200"
                    title="复制结果"
                  >
                  ...
                  <div v-if="!result.error" class="flex items-center space-x-1">
                    ...
                    >
                      <i :class="star <= (result.userRating || 0) ? 'i-lucide-star text-yellow-500' : 'i-lucide-star text-gray-300'" class="w-4 h-4"></i>
                    </button>
                  </div>
                </div>
              </div>
              
              <div v-if="!result.error" class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                <p class="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{{ result.optimizedPrompt }}</p>
              </div>
              
              <div v-else class="bg-red-50 rounded-xl p-4 border border-red-200">
          ...
          <div v-else class="flex flex-col items-center justify-center h-full text-gray-500">
            <i class="i-lucide-sparkles w-16 h-16 mb-6 text-gray-300"></i>
            <p class="text-xl font-semibold mb-3 text-gray-700">等待优化</p>
            <p class="text-sm text-center text-gray-500 leading-relaxed">在左侧输入提示词，选择模型，点击"开始优化"<br>AI将为您生成多个优化版本</p>
          </div>
        </div>
        
        <!-- 底部操作栏 -->
        <div v-if="pageState.results.length > 0" class="p-6 border-t bg-white">
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600 font-medium">{{ successCount }}/{{ pageState.results.length }} 个模型成功</span>
            <div class="flex space-x-3">
              <button @click="saveToHistory" class="btn-secondary text-sm px-4 py-2 rounded-xl">
                <i class="i-lucide-bookmark w-4 h-4 mr-2"></i>
                保存到历史
              </button>
              <button @click="retryOptimization" class="btn-primary text-sm px-4 py-2 rounded-xl">
